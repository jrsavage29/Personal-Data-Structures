 ...
 private:
	 ...
	 int numberofNodes_helper( Node<T>* sub_root )const;
...
/* Given a non-empty binary search tree, return the node with minimum 
   ...
   need to be searched. */
template<typename T>
Node<T>* BST<T>:: find_target( Node<T>* sub_root )
{
	//Go right once and then all the way left. 
	//If no left node then update with the right side node.
...
template<typename T>
int BST<T>:: numberofNodes_helper( Node<T>* sub_root )const
{
	if (sub_root == nullptr) 
	{
       return 0; 
